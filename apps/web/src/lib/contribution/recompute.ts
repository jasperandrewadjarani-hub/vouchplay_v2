...
export async function recomputeAllContributions(): Promise<number> {
  ...
  const svc = createServiceClient();
  const [{ data: vouchRows, count: vouchCount }, { data: flagRows, count: flagCount }] =
    await Promise.all([
      svc
        .from('vouches')
        .select('voucher_id, target_id, created_at', { count: 'exact' })
  ...
  if ((vouchCount ?? 0) > settings.maxFactRows)
    throw new Error('contribution_source_bound_exceeded');
  if (vouchCount !== null && (vouchRows?.length ?? 0) < vouchCount)
    throw new Error('contribution_source_truncated');
  if (flagCount !== null && (flagRows?.length ?? 0) < flagCount)
    throw new Error('contribution_fraud_source_truncated');
  const rows = (vouchRows ?? []) as VouchFact[];
