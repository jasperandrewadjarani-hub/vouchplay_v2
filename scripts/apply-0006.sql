...
-- ---------- Verify ----------
select 'club_tables' as check, count(*) as n from information_schema.tables
  where table_schema = 'public' and table_name in ('clubs','club_memberships')
union all
select 'club_helpers', count(*) from information_schema.routines
  where routine_schema = 'public' and routine_name in ('is_club_member','is_club_manager','is_club_owner')
union all
select 'club_rls_policies', count(*) from pg_policies
  where tablename in ('clubs','club_memberships');
