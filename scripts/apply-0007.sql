...
-- ---------- Verify ----------
select 'tournament_tables' as check, count(*) as n from information_schema.tables
  where table_schema = 'public'
    and table_name in ('tournaments','tournament_organizers','divisions','tournament_interests','tournament_announcements')
union all
select 'tournament_helper', count(*) from information_schema.routines
  where routine_schema = 'public' and routine_name = 'is_tournament_organizer'
union all
select 'tournament_rls_policies', count(*) from pg_policies
  where tablename in ('tournaments','tournament_organizers','divisions','tournament_interests','tournament_announcements');
-- Expect: tournament_tables = 5, tournament_helper = 1, tournament_rls_policies = 8
