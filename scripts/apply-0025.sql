...
-- =============================================================================
-- VERIFICATION - run this as the LAST statement. The Supabase SQL editor only displays the result
-- of the final statement, so this is deliberately one query.
-- Expected: invitation_team_id_column=1, new_rpcs=3, accept_rpc=1, invitation_team_index=1.
-- legacy_open_invitations is informational: invitations already in flight that keep the old path.
-- =============================================================================
select 'invitation_team_id_column' as check, count(*)::int as value
  from information_schema.columns
 where table_schema = 'public' and table_name = 'partner_invitations' and column_name = 'team_id'
union all
select 'new_rpcs', count(*)::int
  from pg_proc p join pg_namespace n on n.oid = p.pronamespace
 where n.nspname = 'public'
   and p.proname in ('create_team_with_pending_partner','decline_partner_invitation','replace_pending_partner')
union all
select 'accept_rpc', count(*)::int
  from pg_proc p join pg_namespace n on n.oid = p.pronamespace
 where n.nspname = 'public' and p.proname = 'accept_partner_invitation'
union all
select 'invitation_team_index', count(*)::int
  from pg_indexes where schemaname = 'public' and indexname = 'idx_partner_invitations_team'
union all
select 'legacy_open_invitations', count(*)::int
  from partner_invitations where status = 'sent' and team_id is null;
