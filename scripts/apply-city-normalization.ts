...
for (const p of rows as { id: string; city: string }[]) {
  ...
  if (sample.length < 20)
    sample.push(
      `${JSON.stringify(p.city)} -> ${next === null ? '(null, test row)' : JSON.stringify(next)}`,
    );
  if (!dry) {
...
console.log(
  `${dry ? 'DRY RUN - ' : ''}profiles with city: ${rows.length} | canonicalized: ${changed} | test rows nulled: ${nulled}`,
);
console.log('sample:');
