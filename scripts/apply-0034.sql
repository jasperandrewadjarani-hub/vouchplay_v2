-- =============================================================================
-- VouchPlay v2 - Migration 0034: STS_V2 columns + settings (Rig-resistant Community Skill, §2AF)
--
...
--
-- What this does: adds six nullable columns to player_skill_profiles so V2 can be computed and
-- stored ALONGSIDE the existing V1 columns on every vouch write, with nothing public changed yet.
-- Additive, nullable, no RLS change (the table's existing RLS already covers these columns) and no
-- backfill - existing rows simply get nulls until the app starts writing V2. App reads are
-- fail-open (the §2R pattern): until this migration is applied, the V2 write inside the recompute
-- path no-ops, so nothing breaks on a live registration event. The public
...
--
-- Apply via the Supabase SQL editor (same method as 0001-0033).
-- =============================================================================
...
-- ---------------------------------------------------------------------------
-- Verify: the 6 new columns exist, and all 18 STS_V2 settings keys are seeded.
-- Expect new_v2_columns=6, skill_v2_settings=18.
-- ---------------------------------------------------------------------------
select 'new_v2_columns' as check, count(*)::int as value
  from information_schema.columns
 where table_schema='public' and table_name='player_skill_profiles'
   and column_name in (
     'community_skill_level_v2','sts_v2','n_eff_v2','weight_sum_v2','components_v2','calculated_v2_at'
   )
union all
select 'skill_v2_settings', count(*)::int
  from system_settings
 where key in (
     'skill_algorithm_active_version','vouch_velocity_guard_enabled',
     'skill_v2_trust_unknown_factor','skill_v2_trust_unanchored_factor',
     'skill_v2_trust_standing_saturation','skill_v2_trust_maturity_days',
     'skill_v2_reciprocal_multiplier','skill_v2_bloc_decay','skill_v2_prior_weight',
     'skill_v2_min_independent_vouchers','skill_v2_velocity_window_hours',
     'skill_v2_velocity_burst_min','skill_v2_velocity_low_trust_share',
     'skill_v2_velocity_low_trust_vt','skill_v2_swarm_min','skill_v2_ring_reciprocal_share',
     'skill_v2_bloc_share','skill_v2_spike_bands'
   );
