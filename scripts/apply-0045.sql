-- =============================================================================
-- VouchPlay v2 - apply 0045: solo-entry partner merge, dismissible bare slots, cancel v3
-- Paste-and-run copy of supabase/migrations/0045_solo_entry_merge_and_cancel_v3.sql, with a
-- verification SELECT appended. Statements are byte-identical to the migration.
--
-- RUN THIS WITH OR AFTER THE §2AT DEPLOY, NOT BEFORE IT. `accept_partner_invitation` now releases a
-- merging player's solo registration and returns `merged_registration_id`; the SERVER moves their paid
-- tournament_slots row onto the joined entry. Run ahead of the deploy, the live code ignores those keys
-- and a merged player's slot sits on a withdrawn registration - invisible in My registrations until the
-- deploy lands. No money is lost (the row is intact and is re-attached on the next settle), and nothing
-- else in this script cares about the order.
--
-- Safe during the open registration window otherwise: one nullable column with no default, one new
-- function, five `create or replace` re-creations that keep their exact signatures and return types.
-- =============================================================================
...
-- ---------- Verification ----------
-- 1. The new column exists and is nullable.
...
-- 2. All six functions exist and are SECURITY DEFINER (prosecdef = true).
select proname, prosecdef, pg_get_function_identity_arguments(oid) as args
 ...
 where pronamespace = 'public'::regnamespace
   and proname in (
     ...
     'player_cancel_registration'
   )
 order by proname;

-- 3. Nothing outside the service role may execute them (EXPECT 0 ROWS). A null proacl means the
--    Postgres PUBLIC-execute default is still in force, which is itself a finding - so it is expanded
--    here rather than skipped.
select p.proname,
       coalesce(a.rolname, 'PUBLIC') as unexpected_grantee
  from pg_proc p
 ...
 where p.pronamespace = 'public'::regnamespace
   ...
   and coalesce(a.rolname, 'PUBLIC') in ('PUBLIC', 'anon', 'authenticated')
 order by p.proname, unexpected_grantee;
