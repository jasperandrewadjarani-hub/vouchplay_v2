-- =============================================================================
-- VouchPlay v2 - apply 0042: tournament slots (pay per seat), play-down-one, coach vouch count
...
--
-- Paste-and-run copy of supabase/migrations/0042_tournament_slots_play_down_coach.sql for the Supabase
...
-- ---------------------------------------------------------------------------
-- Verify. Expect tournament_slots_table=1, allow_play_down_one_level_column=1,
-- coach_vouch_count_column=1, settings_seeded=3, and players_with_coach_vouches = however many
-- players currently hold an active coach-weighted vouch (effectively 0 on 2026-09-12).
-- The index list must show 5 rows: tournament_slots_pkey, idx_tournament_slots_player,
-- idx_tournament_slots_registration, uq_tournament_slots_live_attached, uq_tournament_slots_live_bare.
-- Both functions must come back with prosecdef = true (and are locked down by the revoke/grant pairs
-- above - scripts/check-migration-grants.mjs).
-- ---------------------------------------------------------------------------
select 'tournament_slots_table' as check, count(*)::int as value
  from information_schema.tables
 where table_schema = 'public'
   and table_name = 'tournament_slots'
union all
select 'allow_play_down_one_level_column' as check, count(*)::int as value
  from information_schema.columns
 where table_schema = 'public'
   and table_name = 'tournaments'
   and column_name = 'allow_play_down_one_level'
union all
select 'coach_vouch_count_column' as check, count(*)::int as value
  from information_schema.columns
 where table_schema = 'public'
   and table_name = 'player_skill_profiles'
   and column_name = 'coach_vouch_count'
union all
select 'settings_seeded' as check, count(*)::int as value
  from system_settings
 where key in (
   'profile_show_vouch_meter',
   'profile_show_community_skill',
   'tournament_slot_reservations_enabled'
 )
union all
select 'players_with_coach_vouches' as check, count(*)::int as value
  from player_skill_profiles
 where coach_vouch_count > 0;

select indexname
  from pg_indexes
 where schemaname = 'public'
   and tablename = 'tournament_slots'
 order by indexname;

select proname, prosecdef
  from pg_proc
 where pronamespace = 'public'::regnamespace
   and proname in ('player_fits_division', 'player_cancel_registration')
 order by proname;
