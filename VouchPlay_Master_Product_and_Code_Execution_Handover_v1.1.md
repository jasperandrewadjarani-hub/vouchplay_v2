...
## 42.7 Web Push channel (v1.75, master_plan §2AY)

`WebPushChannel` is no longer "future". `lib/notifications/push.ts` sends a Web Push (VAPID, the
`web-push` library) for every notification row `notify()` / `notifyMany()` inserted - so the
per-category mutes, the critical override and the self-notification skip apply unchanged - scheduled
through Next `after()` so the domain action never waits on it. Devices live in `push_subscriptions`
(owner, endpoint, keys, user agent, last seen, disabled; service-role only); the row IS the opt-in,
saved from the "Notifications on this device" switch; a 404/410 from the push service disables the
device; `push_max_devices_per_user` bounds devices per player. Inert until
`NEXT_PUBLIC_VAPID_PUBLIC_KEY` + `VAPID_PRIVATE_KEY` exist; Admin `push_notifications_enabled` is the
kill switch. Email is unchanged (critical types still email). Payload: title, body, link; tapping the
notification opens the link in the installed app. `ExpoPushChannel` remains future.
...
## 44.1 As built (v1.75, master_plan §2AY)

- **Icons:** generated by `scripts/generate-pwa-icons.mjs` from the rounded "just the V" emblem
  (`public/brand/vouchplay-emblem.png`, neon on solid black; §2AZ addendum, was the sharp
  `vouchplay-logo.png`) on a pure-black ground: `icons/icon-192.png`, `icon-512.png`,
  `icon-maskable-512.png` (80% safe zone), `icons/badge-96.png` (monochrome, Android status bar),
  `app/icon.png` (favicon), `app/apple-icon.png`. Manifest: standalone, portrait, `id: '/'`, brand dark
  `theme_color`, `#000000` `background_color` (matches the icon edge).
- **Service worker:** served by `app/sw.js/route.ts` (source in `lib/pwa/sw-source.ts`) with
  `Cache-Control: no-cache`; the cache name carries the Vercel deployment id so every deploy activates a
  fresh worker and drops old caches. Strategy: navigations network-only, fallback `/offline` (HTML is
  never cached - pages are personalised and skew-protected); `/_next/static/*` cache-first (hashed);
  icons, brand files, manifest stale-while-revalidate; every non-GET, `/api/*`, Supabase and server-action
  request is passed through untouched. Registered post-load by `PwaProvider` (app shell).
- **Offline screen** `/offline`: logo, "You're offline", one line, Retry. No write is ever queued.
...
- **Icon:** the app/install icon now uses the rounded, glossy "just the V" emblem
  (`public/brand/vouchplay-emblem.png`, neon on solid black, wordmark-free) instead of the older
  sharp-edged `vouchplay-logo.png`. `generate-pwa-icons.mjs` rewritten: no wordmark crop, opaque icons
  composite on pure black (so no black square shows), badge silhouette derived from luminance. Manifest
  `background_color` pinned to `#000000` so the install splash matches the icon edge.
- **Persistent install banner:** the banner's dismiss is now session-scoped, not forever - it returns
  each visit until the app is installed, to push every user onto the installed app. Admin kill switch
  and ME-page card unchanged.
- **Notifications on by default at install:** `components/pwa/push-auto-enable.tsx` auto-requests
  permission + subscribes for a signed-in viewer on first standalone launch (the OS permission dialog is
  unavoidable; immediate on Android Chrome, next-tap fallback on installed iOS). Once per device, decline
  respected, gated on `push_notifications_enabled`.
...
- **§44 implemented, not just required:** real app icons (emblem on the brand dark ground, maskable
  safe zone, Android badge, favicon, apple-touch-icon); a service worker served from `/sw.js` with the
  deployment id as its cache version (navigations network-only with an `/offline` fallback - HTML is
  never cached; `_next/static` cache-first; icons stale-while-revalidate; API, Supabase and server
  actions untouched); a branded `/offline` screen with Retry; Admin kill switch
  `pwa_service_worker_enabled` (serves a self-unregistering worker).
- **§27 push channel (new §42.7):** Web Push (VAPID) mirrors every in-app notification `notify()`
  decided to insert - the same per-category mutes apply - and is sent after the response (`after()`),
  never delaying the action. The device subscription is the opt-in (`push_subscriptions`, service-role
  only); a 404/410 disables the device; inert until the VAPID env exists. Kill switch
  `push_notifications_enabled`; `push_max_devices_per_user` (5).
- **Surfaces:** "VouchPlay on your phone" card on the ME page (Install on Android / "Show me how" sheet
  on iPhone / "Open in Chrome or Safari" inside Facebook-Messenger-Instagram browsers; a "Notifications
  on this device" switch whose description is the state, with a confirmation push when turned on); the
  same switch on Notification preferences; a compact "Get updates on your phone" opt-in on the
  registration Done step and the vouch success screen; Admin Operations "Push: n devices" + "Send me a
  test push". No automatic permission prompt anywhere.
- Deferred: app-icon unread badge, per-category push preferences, manifest screenshots, store wrappers.
